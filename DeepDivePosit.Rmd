...
```{r }
Parish$Conflict<- ifelse(grepl("war|fight|sick|scuffle|bloodshed|military|conflict", Parish$text,
                             ignore.case = T), "yes","no")

head(Parish$Conflict)
```

2.  Group by Conflict and geographical area
...
```{r }
ConflictGroup <- Parish %>%
  group_by(Area) %>%
  summarise(Total = n(),
            count = sum(Conflict == "yes")) %>%
...
```{r }
MergedGeo <-merge(ParishesGeo,ConflictGroup,
                  by.x="JOIN_NAME_",
                  by.y="Area",
...
tm_shape(MergedGeo) + # Specify the spatial object (MergedGeo) to be used in the map
  ...
  tm_layout(title = "Conflict reports", legend.text.size = 0.75, legend.title.size = 1, frame = FALSE) # Set layout: add a title, resize legend text and title, remove frame
...
tm_shape(MergedGeo) + # Specify the spatial object (MergedGeo) to be used in the map
  ...
  tm_borders(col = "black") + # Add black borders to each polygon
  tm_layout(title = "Conflict report", legend.text.size = 0.75, legend.title.size = 1, frame = FALSE) # Set layout: add a title, resize legend text and title, remove frame
...
tm_shape(MergedGeo) + # Specify the spatial object (MergedGeo) to be used in the map
  ...
  tm_borders(col = "black") + # Add black borders to each polygon
  tm_layout(title = "Conflict reports", legend.text.size = 0.75, legend.title.size = 1, frame = FALSE, legend.position = c(1, 0.5)) # Set layout: add a title, resize legend text and title, remove frame, position legend at (1, 0.5)
...
```{r }
Parish$Whiski<- ifelse(grepl("illicit still|illicit distillery|drunk|intemperance|wisky|whisky|whiskey|whysky |alembic",Parish$text, ignore.case = T), "yes","no")
```
...
WhiskiGroup <- Parish %>%
  group_by(Area) %>%
  summarise(Total = n(), count = sum(Whiski == "yes")) %>%
  mutate(per = round(count / Total, 2))
...
```{r }
MergedGeo3 <-merge(ParishesGeo,WhiskiGroup, by.x="JOIN_NAME_", by.y="Area",all.x = TRUE) # nb this is left join cause I want to preserve all the records present in ParishGeo
...
tm_shape(MergedGeo3) +
  tm_fill("per",
          ...
          colorNA = "grey") +
  tm_borders(col = "black")+
  tm_layout(title = "Whiski Reports",
            legend.text.size = 0.75,
            legend.title.size = 1,
            frame = FALSE) +
...
```{r}
PointsDistilleries<- st_read(dsn = "Spatial/ScottishDistilleries.gpkg")
```
...
tm_shape(MergedGeo3) + 
  ...
  tm_layout(title = "Whiski Reports",  # Title of the map
